...
import { generatePdfBuffer, type IncidentRow } from "../services/pdf.js";
import fs from "fs";
...
import { protectedProcedure, publicProcedure, router, analystProcedure } from "../_core/trpc.js";
import {
  createLocalUser,
  createIncident,
  deleteIncident,
...
import { registerSchema, loginSchema, createIncidentSchema, validateJoi } from "../lib/validation.js";
import bcrypt from "bcryptjs";
import { v4 as uuidv4 } from "uuid";
import { sdk } from "../_core/sdk.js";
import { notifyOwner } from "../_core/notification.js";
import { publishSiemEvent } from "../integrations/siem/index.js";
import {
  ensureFlaskRunning,
  readMetricsJson,
  classifyIncident,
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (incident) {
        publishSiemEvent({
          ...
          iaMethod: category === "unknown" ? "fallback" : "ml",
        }).catch((err) => console.warn("[SIEM] publish failed:", err));
      }
...
const categoriesRouter = router({
  list: publicProcedure.query(async () => {
    const { listCategories } = await import("../models/db.js");
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const { createCategory } = await import("../models/db.js");
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
      const { updateCategory } = await import("../models/db.js");
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
      const { deleteCategory } = await import("../models/db.js");
...
const adminRouter = router({
  ...
  reclassify: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      publishSiemEvent({
        eventType: "incident.reclassified",
        incident: {
          id: updated.id,
          title: updated.title,
