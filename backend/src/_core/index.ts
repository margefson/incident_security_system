...
const __dirname = __bootstrapDir;
...
async function startServer() {
  ...
  app.get("/api/debug/incidents", async (_req, res) => {
    try {
      const { getAllIncidents } = await import("../models/db.js");
  ...
  // Produção: servir build do frontend. Dev: UI no Vite (5173/5174), API só em /api.
  ...
  if (process.env.NODE_ENV === "development") {
    if (!(await isPortAvailable(preferredPort))) {
      console.error(
        `[API] Porta ${preferredPort} em uso. Libere a porta (ex.: encerre outro "pnpm dev") ou defina PORT e VITE_API_PROXY_TARGET no .env.`
      );
      process.exit(1);
    }
  } else {
  ...
  server.listen(port, () => {
    ...
    console.log(`[API] Backend: ${apiBase} (rotas em /api/trpc)`);
    if (process.env.NODE_ENV === "development") {
      console.log(`[WEB] Abra o frontend: ${process.env.FRONTEND_URL ?? "http://localhost:5173"}`);
      console.log(`[WEB] Proxy Vite → ${process.env.VITE_API_PROXY_TARGET ?? apiBase}`);
    }
  });
}
